...
class TokenStream:
    ...
    def next_doc_and_term(self):
        ...
        while type(self.document) is not bs4.element.Tag:
            if self.document is None:
                if len(self.file_names) is 0:
                    return None
                if len(self.file_names) is not 0:
        ...
        if self.document is None:
            ...
            """ 每一个文档转成 字符串 """
            temp = ''
            for row in dd.document.docno.next_siblings:
...
if __name__ == '__main__':
    dd = TokenStream('/home/superhui/Informationretrieval/DocumentIndexer/data/document')
